...
impl ProgressState {
    ...
    pub fn set_active(&self, slot: usize, sample: &str, step: &str) {
    ...
    pub fn update_step(&self, slot: usize, step: &str) {
        if let Ok(mut jobs) = self.active_jobs.lock() {
            if slot < jobs.len() {
...
pub(crate) fn render_screen(
    ...
    // Header
    let title = format!("STAR-RSeQC v{VERSION}");
    ...
    let cancel_hint = if crate::is_cancelled() {
        "  CANCELLING..."
    } else {
        "  Ctrl+C to cancel"
    };
...
// ─── Duration formatting (public for main summary) ──────────────────────────

pub(crate) fn fmt_duration_pub(d: Duration) -> String {
    fmt_duration(d)
}
