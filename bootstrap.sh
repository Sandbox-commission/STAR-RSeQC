...
DEFAULTS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/star-rseqc/defaults.env"
...
print_reference_env_hint() {
  if [[ -z "$GENOME_DIR" || -z "$GTF_FILE" ]]; then
    return 0
  fi
  echo
  log "Reference environment variables for direct star-rseqc runs:"
  echo "  export STAR_RSEQC_GENOME_DIR=\"$GENOME_DIR\""
  echo "  export STAR_RSEQC_GTF=\"$GTF_FILE\""
  echo
}

configure_reference_env_prompt() {
  if [[ -z "$GENOME_DIR" || -z "$GTF_FILE" ]]; then
    return 0
  fi
  ...
  {
    echo "# STAR-RSeQC persisted defaults (generated by setup.sh)"
    echo "STAR_RSEQC_GENOME_DIR=\"$GENOME_DIR\""
    echo "STAR_RSEQC_GTF=\"$GTF_FILE\""
    if [[ -n "$TOOL_STAR_ENV" ]]; then
      echo "STAR_RSEQC_STAR_ENV=\"$TOOL_STAR_ENV\""
    fi
    if [[ -n "$TOOL_RSEQC_ENV" ]]; then
      echo "STAR_RSEQC_RSEQC_ENV=\"$TOOL_RSEQC_ENV\""
    fi
    if [[ -n "$TOOL_DEEPTOOLS_ENV" ]]; then
      echo "STAR_RSEQC_DEEPTOOLS_ENV=\"$TOOL_DEEPTOOLS_ENV\""
    fi
  } > "$DEFAULTS_FILE"
  success "Saved persisted defaults: $DEFAULTS_FILE"
  ...
  read -r -p "Also append exports to $rc_file? (optional, defaults file already saved) [y/N]: " answer
  case "$answer" in
    y|Y|yes|YES)
      ;;
    *)
      log "Skipped shell profile update. Persisted defaults file will still be used automatically."
      return 0
      ;;
  esac
  ...
  awk '
    $0 !~ /^export STAR_RSEQC_GENOME_DIR=/ && $0 !~ /^export STAR_RSEQC_GTF=/
  ' "$rc_file" > "$tmp"

  {
    echo ""
    echo "# STAR-RSeQC reference defaults"
    echo "export STAR_RSEQC_GENOME_DIR=\"$GENOME_DIR\""
    echo "export STAR_RSEQC_GTF=\"$GTF_FILE\""
  } >> "$tmp"
...
run_conda_mode() {
  ...
  TOOL_STAR_ENV="$cprefix"
  TOOL_RSEQC_ENV="$cprefix"
  TOOL_DEEPTOOLS_ENV="$cprefix"
  configure_reference_env_prompt
  if [[ $RUN_ANALYSIS -ne 1 ]]; then
